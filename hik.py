...
client.publish("homeassistant/binary_sensor/hikcom/motion/config", '{"name": "Motion", "device_class": "motion", "state_topic": "' + config.mqtt_topic + '/status", "value_template": "{{ value_json.motion}}", "payload_on": true, "payload_off": false, "unique_id": "mqtt.hikcom.motion"}', 2, True)
client.publish("homeassistant/binary_sensor/hikcom/ring/config", '{"name": "Ring", "device_class": "sound", "state_topic": "' + config.mqtt_topic + '/status", "value_template": "{{ value_json.ring}}", "payload_on": true, "payload_off": false, "unique_id": "mqtt.hikcom.ring"}', 2, True)
